...
$.extend($.validator, {
	...
	messages: {
		...
		maxTo:"Please enter the biger value again.",
		...
		isidcard:"This is idcard validate",
		isPostalCode:"This is isPostalCode validate",
		isLoginName:"This is isLoginName validate",
		isPhoneCall:"This is isPhoneCall validate"
	},
	...
	classRuleSettings: {
		...
		isidcard: {isidcard: true},
		isPostalCode: {isPostalCode: true},
		isLoginName: {isLoginName:true},
		isPhoneCall: {isPhoneCall: true}
	},
	...
	methods: {
		...
		// http://docs.jquery.com/Plugins/Validation/Methods/maxTo
		maxTo: function(value, element, param) {
			return value > $(param).val();
		},
		isidcard: function(sId, element) {	
			...
			// contributed by Scott Gonzalez: http://projects.scottsplayground.com/iri/
			//return this.optional(element) || /^([0-9]{15}|[0-9]{18})$/i.test(element.value);
		},
		//登录户名
        isLoginName: function(value,element)
        {
            var regTextPost = /^\w+$/;
            return this.optional(element) || regTextPost.test(value);
        },		
		//是否邮政编码(中国)
        isPostalCode: function(value,element)
        {
            var regTextPost = /^(\d){6}$/;
            return this.optional(element) || regTextPost.test(value);
        },
		//是否有效的电话号码(中国)
        isPhoneCall: function(value,element)
        {
            return this.optional(element) || /(^[0-9]{3,4}\-[0-9]{3,8}$)|(^[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^[0-9]{3,4}[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)/i.test(element.value);
        },
        
        //是否为字母和数字        
		stringEN: function(value, element) 
		{   
