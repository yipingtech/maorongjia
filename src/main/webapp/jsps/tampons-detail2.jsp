...
<%--	             alert('wx.error: '+JSON.stringify(res));--%>
  ...
  <body class="s_p_body">
		...
		<section id="s-p-2" class="s-p-2 border-bottom">
			...
			<span>价格：${productInfo.shopPrice}</span><span class="succeed-o s-p-2-span-color">RMB</span>
		...
		<!-- 宝贝详情 -->
		<section class="color-black s-p-4 s-p-4-4" style="display: block;">
				<img alt="" src="${uploadPath }/${productInfo.para12}">
		</section>
		<!-- 宝贝详情 end -->
		<!-- 评论 -->
		       ...
		       function submitProduct2(){
		    	   var repertory = $("#proReper").val()-$("#amount").val();
					if(repertory>0){
						...
						if(buyType=="buyNow"){
							$('#productForm').prop("action","orderInfoAction!addOrder.action?proAttrIds="+proAttrIds);
							$('#productForm').submit();
						}
					}else{
						layer.open({
