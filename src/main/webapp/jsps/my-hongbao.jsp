...
<section class="e-ad">
    ...
    <section class="fenxiao-content">
        <section class="f-c" id="blo">
            ...
            <section class="tab-content" style="border:none;">
                <section id="blo">
                ...
                <div class="fenxiao-scroll">
                    ...
                    <table align="center" class="table-commissionInfo">
                        ...
                        <tr>
                            ...
                            <td class="text-center">${commissionInfo.fromMemberId.nickname}</td>
                            <c:if test="${commissionInfo.commissionStatus ne '4' }">
                                <td class="text-center">${commissionInfo.commissionComments}</td> 
                            </c:if>
                            <c:if test="${commissionInfo.commissionStatus eq '4' }">
                                <td class="text-center">${mtag:cutString(commissionInfo.commissionComments,7,'')}</td> 
                            </c:if>
                            
                            <td class="text-indent-5"><fmt:formatNumber value="${commissionInfo.commission}" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber></td>
                        </tr>
                        </c:forEach>
                    </table>
                    ...
                    <div style="margin-top: 45%;font-size:1.4em">没有红包信息</div>
                    </c:if>
                    </div>
                   </div>
                    <p>*说明：如有差异或疑问，请及时联系百丽春的客服人员。全国热线：<a tel="${standbyList.standby2 }">${standbyList.standby2 }</a>.</p>
                </section>
            </section>
        </section>
	 ...
	 function ajaxNoPayOrder(pageStart){
	    ...
	    function ajaxOrder(pageStart){
				 $.ajax({ 
					...
					str+="<tr><td>"+payOrder.member.loginName+"</td><td class='text-indent-5'>"+(payOrder.payTime).substring(0,10)+"</td><td class='text-indent-5'>"+(json.cashDouble)*(payOrder.orderAmount)+"</td><td class='text-indent-5'>"+payOrder.orderAmount+"</td></tr>";
					});
				 ...
				 $.ajax({ 
					...
					if(json.commissionInfos!=""&&null!=json.commissionInfos){
					 $.each(json.commissionInfos, function(index, commissionInfo) {
					  str+="<tr><td>"+(commissionInfo.addTime).substring(0,10)+"</td><td class='text-center'>"+commissionInfo.fromMemberId.nickname+"</td><td class='text-center'>"+commissionInfo.commission+"</td><td class='text-indent-5'>"+commissionInfo.commissionComments+"</td><td></td></tr>";
					});
